...
#
# File description:
#   Creating the URI and selecting the timespan. This differs too and is shown
#   by the example of Hespress.
...
# Loading corpora.
corpus2011 <- read.csv(
  file = "~/Schreibtisch/Newspaper Corpus/Hespress/hespress2011.csv",
  fileEncoding = "UTF-8", sep = ",",
  header = FALSE, stringsAsFactors = F
)
corpus2010 <- read.csv(
  file = "~/Schreibtisch/Newspaper Corpus/Hespress/hespress2010.csv",
  fileEncoding = "UTF-8", sep = ",",
  header = FALSE, stringsAsFactors = F
)

# To make sure we don't have any empty articles.
...
# First we select all articles written in December 2010
...
# Save the resulting talbe as csv.
...
# Repeat the procedure for Jan-Mai 2011
sp <- dim(dec)[1]
for (i in 1:5) {
