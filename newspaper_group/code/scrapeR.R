...
#
# File description:
#   The functions defined below are used to navigate to the news-sources homepage,
#   identify links leading to articles and save them as HTML in a given target
#   folder. The functions are called from a seperate file, named by the newssource.R
libs<-c("rvest","stringr","tidyr","methods","curl","optparse")
...
##                              THAWRA
###############################################################################
f.scrape.article.thawra <- function(article.homepage, target.folder) {
...
f.scrape.day.thawra <- function(day.homepage.url, target.folder) {
...
##                              AHRAM
###############################################################################
f.scrape.day.ahram <- function(day.to.observe, target.folder) {
  ...
  homepages.rel.v <- f.getLinks(ahram.day.url, "a")
  # also prepends http
  homepages.v <- f.filter_homepages.ahram(homepages.rel.v)
...
f.scrape.article.alwatan <- function(homepage.url.article, target.folder) {
...
f.filter_homepages.ahram<-function(homepages.v) {
...
f.getLinks <- function(homepage.url, link.element) {
